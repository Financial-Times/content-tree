/*
Package contenttree defines the full content tree object.
Provides the functionality for a content tree to be unmarshalled in memory.

Parts of the structs are generated using schema-generate (https://github.com/a-h/generate).
The schema used for the code generation is modified version of schemas/content-tree.schema.json.
The modification applied is dropping blocks of the schema describing arrays of heterogeneous elements.
The result Go structs don't capture fully the data definition in the JSON schemas used for its generation.
There no rules for enums, number of elements in array, etc.

The content tree node types with embedded structs are artificially created, e.g. ListItemChild.
They are used when a node can have heterogeneous children - children with different types.
There is custom UnmarshalJSON method for those artificial types. It is defining a custom logic for unmarshalling object
with embedded objects which implement the same interface. The customization is necessary as the embedded structs
contain essentially duplicate field names (e.g. "Type"). According to the official documentation
(https://pkg.go.dev/encoding/json), if "there are multiple fields, and all are ignored; no error occurs." -
in short the objects are simply not unmarshalled unless custom unmarshalling logic is applied.
*/
package contenttree
...
// Node provides convenient interface to content tree nodes from different types.
// Allows easy traversal of the tree without type casting.
type Node interface {
	GetType() string
	GetChildren() []Node
	GetEmbedded() Node
...
// typedNode is simple struct carrying information only about the type of content tree node.
// It is useful for unmarshalling only the node type.
